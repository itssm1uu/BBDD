...
drop database if exists atletismo_25;
...
/* Miruna Georgiana Matei DAM1 */
-- 1. Muestra las competiciones celebradas en Estados Unidos.
select c.nombre, c.fecha_inicio, c.fecha_fin, c.ubicacion from competicion c 
...
select e.fecha, d.nombre as disciplina, c.nombre as competicion from eventos e 
...
select pais, count(*) as cantidad_atletas from atletas 
    group by pais having count(*) >= 2 
order by cantidad_atletas desc;


-- 4.Muestra para cada carrera de los mundiales del 2023 el mejor tiempo y el peor tiempo
select d.nombre as disciplina, min(r.resultado) as mejor_tiempo, max(r.resultado) as peor_tiempo 
    from resultados r 
...
-- 5.Para cada atleta muestra su nombre y su apellido en una única columna, el
    -- tiempo obtenido y la diferencia de tiempo con el atleta que quedó por delante
    -- para el evento con valor 2 . Descarta aquellos atletas que no tengan tiempo .
select distinct concat(a.nombre, ' ', a.apellido) as nombre_completo, r.resultado, r.puesto, e.id_competicion, 
    r.resultado - (select resultado from resultados where id_evento = r.id_evento and puesto = 1) as diferencia_con_primero
from resultados r 
    join atletas a on r.id_atleta = a.id 
    join eventos e on e.id_competicion = r.id_evento
where r.resultado is not null and r.puesto = 2;


-- 6. Mostrar los nombres de las carreras femeninas de menos de 600 metros.
select nombre from disciplinas
...
select d.nombre as disciplina, e.fecha, c.ubicacion, ifnull(r.resultado, 'No participó') as resultado from disciplinas d 
    left join eventos e on d.id = e.id_disciplina 
    left join competicion c on e.id_competicion = c.id 
    left join resultados r on e.id = r.id_evento
    left join atletas a on r.id_atleta = a.id 
where a.nombre like 'Noah' and a.apellido like 'Lyles' and d.es_masculina = TRUE 
order by e.fecha;